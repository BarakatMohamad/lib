...
class _MyCallScreenWidget extends State<CallScreenWidget>
  ...
  void _handleStreams(CallState event) async {
    ...
    if (event.stream != null) {
      ...
      print(_remoteRenderer?.muted);
      print(_remoteRenderer);
      print(_remoteRenderer?.value);

      print('New stream attached with ID: ${event.stream?.id}');


      // Assuming _videoRenderer is the widget where you are rendering video
      // _remoteRenderer?.srcObject = event.stream;
      print('New stream attached with ID: ${event.stream?.id}');
    } else {
      print('No active stream.');
    }
    // Ensure _localRenderer and _remoteRenderer are initialized
    try {
      if (_localRenderer != null) {
        // Safely attempt to initialize the renderer
    ...
    // Set the stream to the appropriate renderer
  ...
  void callStateChanged(Call call, CallState callState) {
    ...
    print(callState.cause?.cause);
    print(callState.cause?.reason_phrase);

    print(callState.audio);
    // print(callState.stream?.active);

    print(callState.stream?.id);

    print(callState.state);

    print(callState.refer?.accept);
    print(callState.refer?.aor);

    print("Call state changed: ${callState.state}");
    print("Call cause: ${callState.cause?.reason_phrase}");
    print("Audio muted: ${callState.audio}");
    print("Video muted: ${callState.video}");
    print("Stream ID: ${callState.stream?.id}");
